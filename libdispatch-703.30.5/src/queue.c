...
typedef struct dispatch_pthread_root_queue_context_s *
		dispatch_pthread_root_queue_context_t;//t = s *
...
//lihux 这种定义方式还是很有意思的，实测是ok的语法！
int a[] = {[0]=1, [1]=2}
// 6618342 Contact the team that owns the Instrument DTrace probe before
//         renaming this symbol
//下面这个定义了12个全局的queue，奇怪的是编号是从4开始一直到15的.
//而这定义的12个有可以分为普通的queue和overcommit(over used)两类，这两者有什么区别呢？让我们稍后揭晓
//maintenance
//background
//utility
//default
...
#if HAVE_PTHREAD_WORKQUEUE_SETDISPATCH_NP //貌似下面的代码段并不会进入，因为没有该宏定义
...
//dispatch queue的优先级分为以下5等：排序从低到高
//background
//non-interactive
//low
//default
...
//dispatch manage root queue，负责管理，具体有待深究
...
DISPATCH_CACHELINE_ALIGN
//dispatch manage root queue，负责管理，具体有待深究
//在xcode debug的时候，点击暂停键⏸的时候，程序陷入暂停状态的时候，就会发现thread 2 就是当前这个全局的管理queue（但是第二是巧合还是必然，就不得而知了）
...
//最常用的方法，获取一个全局的queue，根据传入的参数（主要是优先级priority）返回相应的global queue
dispatch_queue_t
dispatch_get_global_queue(long priority, unsigned long flags)
{
	...
	case _DISPATCH_QOS_CLASS_MAINTENANCE: //兼容旧代码，忽略、跳过
		...
		break;
#endif // DISPATCH_USE_NOQOS_WORKQUEUE_FALLBACK 根据传入的优先级参数，找到对应的qos（quality of service）
	...
	return _dispatch_get_root_queue(qos, flags & DISPATCH_QUEUE_OVERCOMMIT);//参数处理完毕后，调用真正的获取global queue的方法，同时可知，传入的flags参数，如果有用的话，暂时也只用于告诉方法是去overcommit的全局queue还是普通的全局queue，貌似我们很好用到
}

//DISPATCH_ALWAYS_INLINE的预编译宏是为了保证这个方法能够尽可能高效的被调用，从而节省运行时间
...
//这种情况很罕见，所以通过使用DISPATCH_NOINLINE明确告知编译器这个方法不需做加速优化
